...
function Verbas(col) {
    ...
    this.valor_parcela = function (rubrica, corporacao) {
        if (!isNaN(remuneracao[rubrica])) {
            ...
            valor =
                remuneracao[document.getElementById("posto").value][rubrica];
        }
    ...
    this.vpe = function () {
        mes = getMes(col);
        vpe = this.valor_parcela("vpe", mes);
        return vpe;
    };
    ...
    this.pttc = function () {
        if (document.getElementById("bool_pttc").value == "sim") {
            valor_pttc = rounddown(
                window["verbas" + col].remuneracao_fixa() * 0.3
            );
        } else {
            valor_pttc = 0;
        }
    ...
    this.contr_pensao_militar = function () {
        if (getMes(col) == "03/01/2020") {
            valor_pmil =
                Math.round(
                    this.remuneracao_fixa() * pensao_militar[mes] * 100
                ) / 100;
        } else {
            valor_pmil = rounddown(
                this.remuneracao_fixa() * pensao_militar[mes]
            );
        }
        return valor_pmil;
    };
    this.contr_pensao_militar_adic = function () {
        if (
            document.getElementById("pensao_militar_adicional").value == "sim"
        ) {
            valor_pmad = rounddown(
                window["verbas" + col].remuneracao_fixa() * 0.015
            );
        } else {
            valor_pmad = 0;
        }
    ...
    this.fundo_saude_adicional = function () {
        ...
        valor_fsa =
            parseInt(qtde_fsa) * this.valor_parcela("cotafsa", corporacao);
        return valor_fsa;
    };
    this.pensao_alim = function (tipo = "pensao_alim") {
        pct_pa = document
            .getElementById("percentual_pa")
            .value.replace(/,/g, ".");
        if (isNaN(pct_pa)) {
            alert("Informe um valor numérico");
            pa = 0;
            ...
            if (tipo_pa == "sal_minimo") {
                ...
                base_calculo =
                    this.remuneracao_total() + this.alim() + this.aux_mor();
    ...
    this.pensao_alim_pe = function () {
        if (document.getElementById("pre_escolar_pensao").value == false) {
            ...
            qtde_pa_sobre_pe =
                document.getElementById("pre_escolar_pensao").value;
            valor_pa_sobre_pe =
                qtde_pa_sobre_pe * this.valor_parcela("pre_escolar");
        }
    ...
    this.irrf = function (tipo = "remuneracao") {
        ...
        if (tipo == "remuneracao") {
            base_calculo =
                this.remuneracao_total() - descontos - this.pensao_alim();
        }
    ...
    this.outros_descontos_nao_dedutiveis = function () {
        return this.outros_descontos_dedutiveis(
            "soma_descontos_nao_dedutiveis"
        );
    };
...
function calcular_rubricas(col) {
    ...
    for (row = 0; row < salario_row.length; row++) {
        if (
            salario_row[row].hasAttribute("id") &&
            salario_row[row].id != "mes"
        ) {
...
function mostrar_campos(event, classe) {
    ...
    for (i = 0; i < elementos.length; i++) {
        if (
            (event.target.id == "situacao_funcional" &&
                starter.value != classe) ||
            starter.value == "null" ||
